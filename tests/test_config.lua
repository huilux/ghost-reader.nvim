...
describe("config", function()
  ...
  it("validates distributed mirror layout", function()
    local zero_gap = config.setup({ buffer = { layout = { min_gap_lines = 0 } } })
    assert.equal(0, zero_gap.buffer.layout.min_gap_lines)
    assert.has_error(function()
      config.setup({ buffer = { layout = { min_gap_lines = -1 } } })
    end, "invalid config value at buffer.layout.min_gap_lines: expected non-negative integer")
    assert.has_error(function()
    ...
    assert.has_error(function()
      config.setup({ buffer = { layout = { region_lines = 1 } } })
    end, "invalid config value at buffer.layout.max_lines_per_block: does not fit region_lines after edge_padding")
    local legal_boundary = config.setup({ buffer = { layout = { region_lines = 2, edge_padding = 0 } } })
    assert.equal(2, legal_boundary.buffer.layout.region_lines)
  end)
