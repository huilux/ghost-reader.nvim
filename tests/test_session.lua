...
describe("session", function()
  ...
  it("hard-hides and detaches when a scheduled reflow render throws", function()
    local renders = 0
    package.loaded["ghost-reader.bookshelf"] = {
      open = function(path)
        return { path = path, format = "txt", chapters = { { title = "One", lines = { "one" } } } }
    ...
    package.loaded["ghost-reader.renderer"] = {
      create = function()
        return {
          render = function()
            renders = renders + 1
            if renders > 1 then error("reflow failed") end
            return true
          end,
          hide = function() return true end,
          stop = function() return true end,
    ...
    assert.is_nil(session.get().reader_maps)
    assert.is_true(renders >= 2)
    session.stop()
  end)
